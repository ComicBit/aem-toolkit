...
function urlsMatch(instanceUrl, instancePort, currentUrl) {
  ...
  if (instanceHostPort.hostname !== currentHostPort.hostname) return false;
  ...
  .then((data) => {
    ...
    function addButtons() {
      ...
      if (
        enableEditButton &&
        type === 'author' &&
        !currentURL.includes('/editor.html/') &&
        getWcmModeFromUrl(currentURL) === 'disabled'
      ) {
      ...
      if (
        enablePublishButton &&
        (currentURL.includes('/editor.html/') || getWcmModeFromUrl(currentURL) === 'disabled') &&
        primaryPublish
      ) {
